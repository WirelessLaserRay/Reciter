...
- **词库广场（Deck Hub）生态全面接入与架构重构**：
  - **各大静态仓库与开放平台生态矩阵**：深度整合 Qwerty Learner 词典静态仓库，并在广场底部设立外部开放平台索引（AnkiWeb 官方共享牌组、AnkiChina 中文记忆库社区、Skywind ECDICT 简明英汉词典等），清晰指引学习者获取全学科、全考试的优质记忆库资源
  - **Anki 记忆库精选专区与原生 .apkg 导入**：新增 Anki 记忆库专区，基于 `fflate` 与 `sql.js` (WASM) 纯前端轻量解包引擎，实现零后端直接解析 `.apkg` 内部 SQLite 数据库，智能清洗 HTML 标签与 Cloze 挖空语法，无缝转为 Reciter 本地闪卡
  - **多 CDN 自动容灾链路**：针对国内网络环境设计 `jsDelivr -> Fastly -> GitHub Raw` 三级静态镜像自动回退机制，确保在线词库极速拉取并提升下载成功率
  - **在线自定义直链拉取**：支持输入任意公开可访问的 `.json` 或 `.apkg` 在线链接一键解析入库
- **启发式词性（POS）解析与语义规范化引擎**：
  - 在 `meaning.ts` 中构建完整的词性识别、中英对照清洗与规整系统（`extractAndNormalizeMeaning`）
  - 自动适配并规范化各类开源排版格式：后置括号词性（如 `远程的 (adj.)`、`(noun)`）、前置中括号词性（如 `[n.] 苹果`、`【动】`）、标准前缀词性（如 `vt. 放弃`、`v./n. 放纵`）及行内粘连的多词性复合句（如 `n. 行为...vt. 扮演...vi. 行动`）
- **本地 JSON 词库全格式与字段别名兼容**：
  - `parseJSON` 现已全面支持扁平数组、带有 `cards`/`words`/`items`/`list`/`vocabulary`/`data` 的根对象包装，以及键值对字典对象
  - 广泛兼容开源词典的常用字段别名（词目支持 `front`/`word`/`name`/`term`/`headWord`；释义支持 `trans`/`translation`/`definition`/`back`/`meaning`/`explanation` 等）
- **开源致谢与参考声明**：在 README 中正式设立「鸣谢与开源参考」专栏，全面注明并由衷致谢 Anki、FSRS 算法、墨墨背单词、Qwerty Learner、ECDICT、COCA 语料、Tauri、sql.js 等开源先驱与开放数据源

### Fixed

- **彻底修复词库下载不完全与样词入库缺陷**：解耦卡片抽屉预览的 `sampleWords` 与全量词库数据通道，修复此前内置与远程仅入库 10 张卡片的问题；建立全量数据校验机制，确保数千词库全量入库
- **异步流式写入防冻结优化**：批量入库大词库（如 3,000 ~ 5,000 词）时，采用每 100 词进度通知与微任务休眠让渡 UI 线程，彻底消除长时间导入导致主界面卡死假死的问题
- **内置离线词库全量词性补齐**：通过多源权威字典对离线内置的大学英语四级核心词库（2,607 词）与考研历年真题核心词库（3,728 词）完成 100% 词性注水补齐（0 词条缺失）
- **词库广场卡片预览防重优化**：优化 DeckHub 抽屉预览中词性角标渲染逻辑，当释义本身已包含该词性时不再重复渲染前缀，杜绝 `n. n. 苹果` 现象
- **Windows 原生对话框与外部浏览器唤起**：点击外部生态平台链接时通过系统默认浏览器安全打开，并在词库下载导入完成/失败时调用 Windows 原生 Win32 对话框反馈
...
- **每日一文付费源多网关解析降级网络**：客户端与 Worker 服务端统一构建四级解析网关（原站直连伪装爬虫 -> Jina Reader 结构化提取 -> Archive.today 网页快照 -> Wayback Machine 历史存档），实现针对强反爬与付费墙文章（如 NYT、WSJ、FT、The Economist）的多级自动穿透
- **付费墙与全文截断特征识别**：增加付费墙、订阅限制及残篇识别检测器，在阅读视图顶部即时展示友好警示卡片与外部快照直达链接
- **解析通道控制栏**：在文章卡片头部提供通道切换菜单（自动级联优化 / 原站直连 / Jina Reader / Archive.today / Wayback）与生效状态胶囊，支持一键重新解析
- **生词识别自主删除与管理**：生词识别列表中每一项支持单项悬浮快速删除（`X` 按钮）与列表顶部一键「清空全部」，点击时不误触讲解卡片
- **手动添加生词多级查词与队列暂存**：
  - 手动添加生词时自动调用本地词库直查、AI 深度讲解、有道开放建议接口以及翻译兜底，自动填充词性与中文释义
  - 新增生词进入待确认暂存队列（不含音标），用户审查确认后再一键批量导入「每日一文生词」词库
  - 离开文章（切换文章、关闭/收起文章、切换新闻源/分类或离开页面）时，自动清空生词添加队列，防止跨篇状态污染

### Changed

- **侧边栏版本号更新**：侧栏底部版本信息同步升级至 `Reciter v0.16.6 · Phase 7`
- **备考计划（Exam Planner）逻辑升级**：复习范围全面涵盖所有今日到期词汇，彻底解决计划学完后仍有待复习词汇残留的问题
- **PWA 与移动端布局体验调优**：优化移动端底部导航栏、背词主卡片与弹窗的间距与安全区排版
- **学习打卡热力图深浅动态化**：依据当日实际学习与复习强度自适应映射热力图色阶，直观展示用度起伏
...
- **词库主页全局卡片综合检索**：在词库页面直接内嵌全局搜索输入框，支持跨词库搜索单词拼写、中文释义及标签
- **Windows 桌面端拼写测试全面适配**：在桌面端强化拼写测试键盘输入响应与严格判定

### Fixed

- **拼写测试检查严格性**：修正拼写判分逻辑在多余空格、大小写和标点符号边界情况下的判定行为
- **下拉选择框（Select）层级优化**：修复移动端与弹窗内下拉菜单被底层遮挡或溢出视图的问题
...
- **覆盖前本地安全快照与一键撤销（Safety Backup & Undo）**：在执行任何云端下载覆盖或本地备份恢复前，系统自动在本地（IndexedDB/内存）建立完整数据镜像；若误操作覆盖，设置页即时显示黄色提醒条并支持一键无损撤销回退
- **Schema 弹性适配与版本降级迁移管道**：新增 `sanitizeBackupData` 弹性清洗转换层，对历史版本或跨端导出的缺失列（`meaning_primary`、`meaning_secondary`、`ignored`、`weak_source`、`weak_dismissed`、`phonetic`）、残损标签及缺省 FSRS 状态字段进行全量防御性兜底，保证新老版本互相导入不报错
- **多端推送冲突主动拦截与时间戳比对**：在上传云端前主动拉取并比对云端最新快照生成时间与本地上次同步记录；若发现其他设备已更新云端，弹出高危警示与时间明细，必须二次确认方可强制覆盖，彻底防止学习进度静默蒸发
- **设置项（Settings）设备本地保护白名单**：建立 `DEVICE_PRESERVED_SETTINGS` 机制，在恢复数据时严格隔离保护 `sync_endpoint`、`sync_token`、`sync_last_remote_time` 等本地设备状态，杜绝多端凭据冲刷与断连风险

### Changed

- **Tauri 桌面端数据恢复全链路失败保护**：修复 Windows 桌面端恢复中途抛错时因缺少快照而导致原库被彻底清空的隐患，改由安全镜像在异常时自动触发二次原子回滚
- **Cloudflare Worker 同步响应元数据暴露**：在 CORS 中暴露并在 `GET /api/sync/snapshot` 响应头带回 `X-Snapshot-Updated-At`，使客户端单次请求即可获取快照与其精确更新时间
- **设置页数据同步状态双向回显**：在同步控制区清晰展现「云端最新快照时间」与「本机上次同步记录」，并优化下载覆盖前二次确认交互
...
- **每日一文生词全面识别**：解除文章 AI 识别单词数量上限（移除原有 12 词限制），支持完整提炼整篇文章核心生词与重难点短语

### Fixed

- **卡片编辑弹窗超出编辑显示框**：修复 `DeckDetail.tsx` 中增加多释义例句后高度超出屏幕视口导致底部输入框与按钮无法操作的问题，改为最大视口固定容器、顶部 Header 与底部 Footer 常驻、中间表单自适应滚动，并将主要/次要释义栅格化双列排布
- **每日一文返回列表溢出截断**：修复 `DailyArticle.tsx` 中列表项长标题与摘要文本因未限制最小宽度而被无省略号生硬裁切的问题，规范单行省略（`truncate`）与完整提示气泡
- **异步音标/释义拆分切路由中断**：重构为全局 `useTaskStore` 状态单例执行后台任务，逐卡片增量写库，彻底解决切换页面组件卸载导致未完成任务被杀和进度丢失的问题
- **每日一句每日自动更新失效**：修复 `daily-quotes.ts` 中周缓存匹配机制，规范为基于自然日日界（YYYY-MM-DD）精确缓存与自动失效换新，并优化 ZenQuotes 网络请求与超时重试
...
- **每日一文 AI 助手栏吸顶与独立滚动**：助手侧栏支持随文章滚屏常驻停靠在视口右上方（`sticky`），并在侧栏内容超长时支持内部独立鼠标滚轮滑动浏览
- **手动添加生词自动写入释义**：在每日一文手动输入单词后，自动通过 AI 词汇讲解（若离线自动回退 Free Dictionary + 翻译接口）查询并填入词性与中文释义，收录与导入词库更省心
- **AI 阅读选择题选中状态高对比高亮**：优化 AI 出题中用户点击选中的选项视觉样式，以高对比主题强调色和阴影光环突出，答案判定后直观标识正误
- **卡片多释义例句异步匹配与预加载**：新增 `card-examples.ts` 模块，支持通过 AI 严格针对卡片的不同释义分别生成最多 3 句地道英文例句与中文翻译（无 AI 时自动通过 Free Dictionary API + 机器翻译降级），并在词库详情页提供一键匹配与进度取消管理
- **例句结构化标签存储与业务隔离**：例句以 `ex:{"sense":...,"en":...,"cn":...}` 结构化存储于卡片标签中，`db.ts` 自动隔离例句标签，保证普通分类与测试筛选不受污染
- **学习页面秒级免网络例句直出**：`DictionaryExample.tsx` 优先读取卡片内预匹配的释义例句，零延迟直接渲染释义标签、英汉对照与发音朗读
- **顶部全局后台任务胶囊**：`Header.tsx` 实时监控并展示后台正在运行的音标丰富、释义分类与例句匹配任务，支持点击直达词库与一键取消
...
- **每日一文逐段对照翻译**：全文翻译按段落左右一一对应
- **每日一文手动添加生词**：可自定义把文章中的生词加入生词收录列表
- **考试规划设置**：设置页可配置考试日期与目标词库，主页倒计时按所选词库计算建议每日新学量
- **AI 备考计划**：基于考试日期、词库数量与学习偏好，由 AI 生成分阶段学习计划
- **每日一句在线化**：接入 ZenQuotes API，每周同步一次英文名言，并用 AI 生成中文翻译；本地数据库缓存，离线自动回退内置名句库，支持手动「换一句」

### Changed

- **README 全面重写**：补充功能矩阵、架构、数据库、项目结构、构建 / PWA / Worker 部署、同步与测试说明
- **主页考试倒计时文案**：显示“距离考试还有 X 天”
- **每日一句来源切换**：改用 ZenQuotes API，并支持 Tauri 环境自适应请求

### Fixed

- **PWA 左上角图标不显示**：改用 `BASE_URL` 拼接图标路径，兼容 GitHub Pages 子路径部署
...
- **学习休息机制**：设置页新增最大单轮学习数量（默认 100）和休息锁时长（默认 15 分钟）；达到上限后提醒休息并锁定学习
- **忽略标签支持正则**：学习忽略标签改为一行一个，支持正则模糊匹配
- **释义主次拆分**：导入时自动识别加粗主要释义与非加粗次要释义，次要释义自动承接词性
- **AI 扫描词库释义**：词库详情可按当前标准（默认考研）用 AI 拆分主要/次要释义
- **学习跳过/忽略**：学习时可跳过（本轮稍后重插）或忽略（永久排除，可在词库恢复）
- **每日一文截断字符数可调**：设置页可自定义文章截断长度（默认 30000）
- **每日一文列表增强**：支持显示全部结果、分页、按时间排序；全文翻译后自动切换左右中英对照
...
- **学习退出行为**：退出学习后清空未完成队列并回到词库选择
- **每日一文左上角标题**：与其他页面统一，左上角显示「每日一文」
- **AI 提示词全面优化**：强化输出约束、筛选原则、考研阅读出题标准、词性保留与防词义丢失
- **应用图标更新**：PWA/页面图标改为 `public/icon.png`
- **移除未使用的 AI 提示词编辑入口**：设置页不再提供提示词编辑与“AI 出题与判分”说明
...
- **Tauri 旧库缺少新列**：注册迁移 010，修复旧库 `meaning_primary/meaning_secondary/ignored` 缺失
- **AI 扫描释义丢失词性/词义**：保留词性并防止内容丢失
...
- **学习/测试快捷键**：回车统一为提交/确定；Y/N 为确定/不确定；测试选择题 1-4 对应选项 A-D；空格默认切页/继续；界面增加快捷键提示
- **跨端全量快照同步**：设置页可配置 Cloudflare Worker + KV 同步地址/Token，支持上传/下载完整备份，PWA 与 Windows 可互相同步
- **每日一文**：侧边栏独立入口；来源为 China Daily / CNN / The Guardian / NPR / BBC；AI 出题（选择题+中文解析+全文翻译）、生词识别/讲解/导入词库、文章收藏；词汇标准可调（四级/六级/考研/专业英语）
...
- **每日一文来源更新**：来源为 China Daily / CNN / The Guardian / NPR / BBC；China Daily 使用 opinion + world 双 RSS
...
- **学习界面音标显示迟滞**：音标改为同步派生（卡片字段优先 + 异步缓存），并预加载队列前三张的音标，避免 Windows/PWA 切换卡片时音标晚出现
...
- **队列例句预加载**：进入学习时预先加载队列前 3 个单词/词组的例句
- **AI 测试计时**：只有真正完成测试才记录“上次测试时间”，中途退出不会刷新提醒
- **TTS 播放与音标显示**：发音增加 Google TTS 音频兜底；单词音标自动从词典接口获取并显示（词组不显示音标）
- **发音/音标布局**：音标显示在单词下方，发音按钮放在单词旁边
- **音标/发音预加载**：队列前 3 个单词的例句、音标、发音都会提前预加载
- **发音来源设置**：设置页可选择 自动 / 系统 TTS / Google TTS
- **单词居中 + 发音图标在旁**：单词保持居中，发音按钮紧挨单词右侧
- **音标解析规则**：忽略括号；含斜杠时只取斜杠前内容；含空格不解析（词组不显示音标）
- **音标与词库绑定**：导入时自动解析并保存音标，学习时直接展示；启动时自动回填已有词库的音标
- **修复导入建库**：同一词库的卡片导入时复用同一目标词库，避免“一卡一词库”
- **修复发音不播放**：系统 TTS 无可用语音时自动切换 Google TTS
- **修复发音按钮被裁切**：单词旁发音按钮改为网格布局，确保始终可见
- **词库详情显示音标**：词库卡片列表在单词下方展示音标
- **词库缺音标检测与补齐**：自动提示缺少音标的单词数量，可一键从词典补齐
- **词库单词旁发音按钮**：词库详情每行单词旁可直接发音
- **导入自动补音标**：导入时若内容没有音标，会自动从词典获取并写入卡片
- **音标获取成功率提升**：词典接口无结果时自动用 AI 生成 IPA 音标兜底
- **词库列表 UI 调整**：表头改为“单词/词组”，音标显示在单词正下方，重点星标不影响音标对齐
- **批量音标获取**：词库补齐/导入时并发获取音标，避免逐条慢请求
- **导入进度显示**：导入时显示“获取音标 / 写入数据库”进度条
- **学习时惰性补音标**：卡片无音标时在线查询并回写数据库，后续直接使用
- **导入/补音标全屏进度提示**：显示进度条并提示“可能需要较长时间，请勿切换页面或关闭窗口”
- **音标解析规则收紧**：只要内容含空格就不解析音标
- **移动端 TTS 优化**：优先系统语音；Google 音频失败时自动用有道音频兜底
...
- **“已掌握”判定放宽**：稳定性阈值从 15 天降到 7 天，且仍要求遗忘次数低于弱词阈值；更多单词能进入「已掌握」状态
- **学习模式例句展示补全**：新卡教学、快速测试、AI 攻克视图都会展示词典/AI 例句；已有原文语境时显示“补充例句”
- **例句带中文翻译**：词典/Tatoeba/AI 例句统一补中文翻译；AI 生成闪卡时也要求输出 `example_cn`
...
- **词库导出选择按钮美化**：勾选/未勾选改为与编辑/删除一致的图标按钮
- **AI 闪卡生成增加词性与例句**：AI 生成的卡片带 `pos`（词性）和 `example`（例句），例句进入原文语境
- **音标/TTS 入口更明显**：学习页卡片上方常驻「发音」按钮，有音标时显示音标

### Fixed

- **TTS/音标在部分学习模式下不可见**：现在所有学习模式都能看到发音入口
...
- **词库批量整合导出**：支持勾选多个词库导出为 JSON，保留标签、重点标记与文件夹信息
- **TTS 发音与音标**：学习卡支持发音按钮，卡片新增音标字段
- **Easy Days 负载均衡**：设置页可开启周末复习量减半
- **AI 智能生成闪卡**：导入页可从文章/笔记让 AI 提取单词并生成卡片
- **考试日期规划**：主页显示考试倒计时与每日建议新学数量
- **性能与数据修复**：随机干扰项取样、响应时间封顶、主动回忆支持英文释义、标签 json_each、Set 去重、DST 安全等
...
- ✅ `npm run build` 通过
- ✅ `.install/6c-test.ts` 全过（Phase 7A-F 后回归验证）
- ✅ `.install/phase7-test.ts` 全过（Phase 7 新功能单测）
