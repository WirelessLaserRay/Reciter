...
**当前进度**：Phase 1-5 + 6A/6B/6C 全部完成（0.14.2，学习逻辑已按 `research_doc/learning_logic_audit.md` 实施全部 13 项改进；含危险区重置、形近词干扰项、选择题自适应选项与侧栏平滑折叠动画，并修复 learning_steps 持久化与同族词误判）。后续方向：Easy Days 负载均衡、更多题型、备份加密、多端迁移、FSRS-6 升级预留。
