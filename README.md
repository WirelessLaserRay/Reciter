...
| 功能 | 说明 | 状态 |
|---|---|---|
| 🗂️ 灵活数据导入 | Markdown / CSV / JSON / TXT 批量导入，支持文本框粘贴手动输入；JSON 重名冲突 diff 与选择性导入；导入前预览、冲突检测 | Phase 2 / 7 |
| 🧠 科学记忆系统 | FSRS-5 间隔重复算法（ts-fsrs v5），目标记忆率可调（0.80~0.95），Learning/Review/Relearning 三态流转，评分按钮实时显示下一间隔，悬停查看四档含义 | ✅ Phase 3 |
| 📝 测试模式 | 填空（中译英）+ 选择（中译英/英译中）三种题型，英文单词选项优先使用形近词干扰项，本地与 AI 选项均随机打乱，掌握度评价回填 FSRS；支持 AI 出题 | ✅ Phase 3 / 0.14.6 |
| 🏷️ 词库管理 | 词库重命名、词条编辑（单词/释义/标签）、按标签分类学习与测试、按词库乱序学习、文件夹分类（跨文件夹同名词库） | ✅ 0.14.6 / 7 |
...
| 🧬 统一学习流 | 按 FSRS 状态自适应切换：新卡教学（先教，延迟突击测试）→ 主动回忆 / 快速测试（秒答阈值可调，形近词干扰）/ AI 深度攻克 / 经典翻转；复习揭示原文语境与同族词；词库掌握度全景、标签集巩固测试；Leech 自动标记重点词 | ✅ 6C / 0.14.6 |
...
| 💾 本地优先 | 所有数据存本地 SQLite（6 表 + 迁移管理）；全量 JSON 导出/恢复；支持多词库批量整合导出 JSON（保留标签/重点/文件夹，不包含 WebDAV 同步） | ✅ Phase 2 / Phase 5 / 7 |
| 🔊 发音与音标 | 学习卡 TTS 发音按钮，支持音标字段展示 | ✅ Phase 7 / 0.15.0 |
| ⚖️ Easy Days 负载均衡 | 开启后周末复习量默认减半，避免堆积（对标 Anki 2025 新特性） | ✅ Phase 7 / 0.15.0 |
| 🤖 AI 智能生成 | 从文章/笔记自动提取单词生成闪卡，支持预览后导入词库 | ✅ Phase 7 / 0.15.0 |
| 🎯 考试日期规划 | 设置考试日期后主页显示倒计时与每日建议新学数量 | ✅ Phase 7 / 0.15.0 |
...
- **安装分发**：运行 `Reciter_0.14.6_x64-setup.exe` 安装到系统，获得开始菜单/桌面图标与卸载程序
