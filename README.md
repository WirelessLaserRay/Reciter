...
| 📝 测试模式 | 填空（中译英）+ 选择（中译英/英译中）三种题型，英文单词选项优先使用形近词干扰项，本地与 AI 选项均随机打乱，掌握度评价回填 FSRS；支持 AI 出题 | ✅ Phase 3 / 0.14.5 |
| 🏷️ 词库管理 | 词库重命名、词条编辑（单词/释义/标签）、按标签分类学习与测试、按词库乱序学习 | ✅ 0.14.5 |
...
| 🧬 统一学习流 | 按 FSRS 状态自适应切换：新卡教学（先教，延迟突击测试）→ 主动回忆 / 快速测试（秒答阈值可调，形近词干扰）/ AI 深度攻克 / 经典翻转；复习揭示原文语境与同族词；词库掌握度全景、标签集巩固测试；Leech 自动标记重点词 | ✅ 6C / 0.14.5 |
...
- **安装分发**：运行 `Reciter_0.14.5_x64-setup.exe` 安装到系统，获得开始菜单/桌面图标与卸载程序
