...
| 🧬 统一学习流 | 按 FSRS 状态自适应切换：新卡教学（识别首测）→ 主动回忆 / 快速测试（秒答阈值可调）/ AI 深度攻克 / 经典翻转；复习揭示原文语境与同族词；词库掌握度全景、标签集巩固测试；Leech 自动标记重点词 | ✅ 6C / 0.13.0 |
| 🎨 多主题外观 | 7 套完整主题（石墨黑/珍珠白/深海蓝/森林绿/星夜紫/暖阳橙/玫瑰红），侧栏折叠动画与图标栏 | ✅ 0.12.0 |
...
- **安装分发**：运行 `Reciter_0.13.0_x64-setup.exe` 安装到系统，获得开始菜单/桌面图标与卸载程序
